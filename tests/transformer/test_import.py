...
import_errmsg = (
    'Line 1: "{name}" is an invalid variable name because it starts with "_"')
...
def test_RestrictingNodeTransformer__visit_Import__2(c_exec):
    ...
    assert result.errors == import_errmsg.format(name='_a')
...
def test_RestrictingNodeTransformer__visit_Import__4(c_exec):
    ...
    assert result.errors == import_errmsg.format(name='_m')
...
def test_RestrictingNodeTransformer__visit_Import__7(c_exec):
    ...
    assert result.errors == import_errmsg.format(name='_n')
...
def test_RestrictingNodeTransformer__visit_Import__9(c_exec):
    ...
    assert result.errors == import_errmsg.format(name='_leading_underscore')
