...
def _compile_restricted_mode(
    ...
    if policy is None:
        ...
        byte_code = compile(
            source, filename, mode=mode, flags=flags, dont_inherit=dont_inherit
        )
        ...
        if IS_PY2:
            allowed_source_types.append(
                unicode  # NOQA: F821,E501  # PY2 only statement, in Python 2 only module
            )
    ...
    return CompileResult(
        byte_code, tuple(collected_errors), collected_warnings, used_names
    )
...
def compile_restricted_function(
    ...
    wrapper_ast = ast.parse(
        'def masked_function_name(%s): pass' % p, '<func wrapper>', 'exec'
    )
